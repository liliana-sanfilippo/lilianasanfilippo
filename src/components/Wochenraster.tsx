...
import {
    Befund,
    Eintrag,
    KollisionsBerichtProps,
    Platziert,
    Schwere,
    WOCHENTAG_KURZ
} from "../types/additional_unibi_types";
...
export function Wochenraster({kohorte, konflikte, quelleModelle}: KollisionsBerichtProps) {
    const daten: any = useMemo(() => {
    ...
    const {jeTag, min, max, maxTag, schwereJeAngebot} = daten;
    ...
    return (
        <>
            <p className={"hinweis"}>
                ...
                {' '}gefüllte Fläche = geprüfter Kurs, schraffiert = harte Überschneidung.
            ...
            >
                ...
                {tage.map((t) => (
                    <div className={"spalte"} key={`spalte-${t}`}>
                        ...
                        {(jeTag.get(t) ?? []).map((e: any, index: number) => {
